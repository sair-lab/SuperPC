...
#-------------------------------------------Single Branch Test Model:ResNet50 + PointNet----------------------------------------
import torch
import torch.nn.functional as F
from torch import nn
from PIL import Image
...
class PointNetEncoder(nn.Module):
    def __init__(self, zdim, input_dim=6):
        ...
        # Mapping to [c], cmean
        self.fc1_m = nn.Linear(2048, 2048)
        self.fc2_m = nn.Linear(2048, zdim)
        self.fc_bn1_m = nn.BatchNorm1d(2048)
        self.fc_bn2_m = nn.BatchNorm1d(zdim)
    ...
    def forward(self, x, img):
        ...
        x = x.transpose(1, 2)                      # ([8, 6, 52000])
        x = F.relu(self.bn1(self.conv1(x)))        # ([8, 128, 52000])
        x = F.relu(self.bn2(self.conv2(x)))        # ([8, 256, 52000])
        x = F.relu(self.bn3(self.conv3(x)))        # ([8, 512, 52000])
        x = self.bn4(self.conv4(x))                # ([8, 2048, 52000])
        x = torch.max(x, 2, keepdim=True)[0]       # ([8, 2048, 1])
        x = x.view(-1, 2048)                        # ([8, 2048])

        m1 = F.relu(self.fc_bn1_m(self.fc1_m(x)))  # ([8, 2048])
        m1 = F.relu(self.fc_bn2_m(self.fc2_m(m1))) # ([8, zdim])
