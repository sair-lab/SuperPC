...
def load_h5_data(args):
    ...
    input = input + 0.02 * np.random.randn(*input.shape).astype(np.float32)
...
def load_h5_data_v2(args, data_path):
    num_points = args.num_points
    num_4X_points = int(args.num_points * 4)
    num_out_points = int(args.num_points * args.up_rate)
    skip_rate = args.skip_rate
...
def random_mirror_point_cloud_and_gt(input, gt=None, z_axis=False):
    """ Randomly rotate the point clouds to augument the dataset
        rotation is per shape based along up direction
        Input:
          Nx3 array, original batch of point clouds
    ...
    Rz = np.array([[-1, 0, 0],
                   ...
                   [0, 0, -1]])
    if z_axis:
        input = np.dot(input, Rx)
        if gt is not None:
            gt = np.dot(gt, Rx)
    else:
        input = np.dot(input, Rz)
        if gt is not None:
            gt = np.dot(gt, Rz)
